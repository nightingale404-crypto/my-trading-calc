...
default_sl = 0.200
default_rr = "4:1"
default_rr_custom = 3.0

# 取得當前腳本所在的資料夾絕對路徑，確保讀取到同資料夾下的 config.json
...
if os.path.exists(config_path):
    try:
        with open(config_path, "r", encoding="utf-8") as f:
            ...
            default_rr_custom = float(cfg.get("rr_custom_val", 3.0))
    except Exception:
        pass

def save_config(capital, risk, stop_loss, rr_ratio, rr_custom_val):
    try:
        cfg = {}
        if os.path.exists(config_path):
            with open(config_path, "r", encoding="utf-8") as f:
                cfg = json.load(f)
        cfg["capital"] = str(capital)
        cfg["risk"] = f"{risk}%"
        cfg["stop_loss"] = f"{stop_loss:.3f}"
        cfg["rr_ratio"] = rr_ratio
        cfg["rr_custom_val"] = rr_custom_val
...
with col_calc:
    ...
    with col_c:
        ...
        if rr_selection == "自訂 (Custom)":
            rr_val = st.number_input("自訂比例 (:1)", value=default_rr_custom, step=0.5, format="%.1f")
        else:
            try:
    ...
    # 自動儲存最新設定（本金、風險、止損、盈虧比）到 config.json，以便下次開啟時載入
    current_rr_custom = rr_val if rr_selection == "自訂 (Custom)" else default_rr_custom
    if (capital != default_capital or 
        risk_pct != default_risk or 
        sl_pips != default_sl or 
        rr_selection != default_rr or 
        current_rr_custom != default_rr_custom):
        save_config(capital, risk_pct, sl_pips, rr_selection, current_rr_custom)
